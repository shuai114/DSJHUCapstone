...
load("gram4.part1.twits_blogs.RData")
load("gram4.part2.twits_blogs.RData")
gram3<-c(gram4.part1.twits_blogs,gram4.part2.twits_blogs)
rm(gram4.part1.twits_blogs,gram4.part2.twits_blogs)
gram3<-gram3[gram3>1]
load("gram3.part1.twits_blogs.RData")
load("gram3.part2.twits_blogs.RData")
gram2<-c(gram3.part1.twits_blogs,gram3.part2.twits_blogs)
rm(gram3.part1.twits_blogs,gram3.part2.twits_blogs)
gram2<-gram2[gram2>1]
load("gram2.part1.twits_blogs.RData")
load("gram2.part2.twits_blogs.RData")
gram1<-c(gram2.part1.twits_blogs,gram2.part2.twits_blogs)
rm(gram2.part1.twits_blogs,gram2.part2.twits_blogs)
gram1<-gram1[gram1>1]
save(gram1,gram2,gram3,file="twits_blogsgrams.RData")

begintime<-Sys.time()
begintime
load("gram2.part1.twits_blogs.RData")
load("gram2.part2.twits_blogs.RData")
unigram<-c(gram2.part1.twits_blogs,gram2.part2.twits_blogs)
rm(gram2.part1.twits_blogs,gram2.part2.twits_blogs)
terms<-strsplit(names(unigram)," ")
terms<-sapply(terms, function(words) words[2])
names(unigram)<-terms
unigram<-tapply(unigram,terms,sum)
rm(terms)
save(unigram,file="unigram_twits_blogs.RData")
Sys.time()-begintime
...
library(RTextTools)
memory.limit(30000)
load("twits_blogsgrams.RData")
load("unigram_twits_blogs.RData")
name1<-names(gram1)
name2<-names(gram2)
name3<-names(gram3)
...
#get the final data (code is written in FinalDataGeneration.R)
#(this data is not used, because it is hard to evaluate and too big)

#get the final data (thin version)
#(just need to correct the names)
setwd("C:/shuai personal stuff/My documents/jobs/en_US/finaldata/training")
memory.limit(30000)
...
gram2<-gram1
gram1<-unigram
save(gram1,gram2,gram3,gram4,file="finaldata_thin.RData")

#old trymatch functions
trymatch<-function(head,trmfreq,trmname,lasttrm){
regexpr<-paste0("(^",head,"| ",head,") [a-z]+$")
matchedindex<-grep(regexpr,trmname)
if (length(matchedindex)>0) {
matchedword<-lasttrm[matchedindex]
matchedTF<-trmfreq[matchedindex]+1/(gram1[matchedword]+1)
names(matchedTF)<-matchedword
matchedTF<-sort(matchedTF,decreasing=T)
matchednames<-names(matchedTF)
} else {matchednames<-character()
...
trymatch<-function(head,trmfreq,trmname,lasttrm){
regexpr<-paste0("(^",head,"| ",head,") [a-z]+$")
matchedindex<-grep(regexpr,trmfreq[,"name"])
if (length(matchedindex)>0) {
trmfreq<-trmfreq[matchedindex,]
matchedword<-trmfreq[,"lastword"]
matchedTF<-trmfreq[,"gram"]+1/(gram1[matchedword]+1)
names(matchedTF)<-matchedword
matchedTF<-sort(matchedTF,decreasing=T)
matchednames<-names(matchedTF)
} else {matchednames<-character()
